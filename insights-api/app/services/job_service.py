...
class JobService:
    ...
    async def create_and_start_job(
        ...
        if self.arq_pool:
            ...
            await self.update_job_status(job_id=job_id, status=JobStatus.FAILED, error="No ARQ pool available")
            return
    ...
    async def _enqueue_tasks_for_job_type(
        ...
        if job_type == JobType.REVIEW_SCRAPING:
        ...
        elif job_type == JobType.PRODUCT_DISCOVERY:
            # Enqueue product discovery task
            task_config = config or {}
            task_config.update({
                "target_type": target_type.value,
                "target_id": target_id,
            ...
            logger.info(f"Enqueuing product discovery task for job {job_id}")
            await self.arq_pool.enqueue_job(
                "discover_products_task",
                job_id=job_id,
                organization_id=organization_id,
                config=task_config
            )
